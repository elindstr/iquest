const getNewIQ = (initialIQ, quizResult) => {
  const exp60 = 0.1;      // at IQ <60, we expect 10%
  const exp120 = 0.5;     // at IQ =120, we expect 50%
  const exp200 = 0.8;     // at IQ >200, we expect 80%
  const scoreRateChange = 0.3; // how quickly the IQ can change
  ...
  let newIQ = initialIQ * adjustmentFactor;

  // limit IQ change to not exceed a 10 point swing
  const maxChange = 10;
  if (newIQ > initialIQ + maxChange) {
    newIQ = initialIQ + maxChange;
  } else if (newIQ < initialIQ - maxChange) {
    newIQ = initialIQ - maxChange;
  }
